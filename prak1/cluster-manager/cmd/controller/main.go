...
type Replica struct {
	ID     string    `json:"id"`
	Agent  string    `json:"agent"` // например "127.0.0.1:8082"
...
var (
	...
	desiredReplicas = 0
	agents          = make(map[string]bool) // "127.0.0.1:8081" → alive
)
...
func registerHandler(w http.ResponseWriter, r *http.Request) {
	// Ожидаем: /register?port=8082
	port := r.URL.Query().Get("port")
	if port == "" {
		port = "8081"
	}

	// RemoteAddr выглядит как 127.0.0.1:51234 → берём только IP
	...
	for {
		...
		for agent := range agents {
			...
			if err != nil || resp.StatusCode != 200 {
				...
				agents[agent] = false
				// Удаляем все реплики этого агента
				var alive []Replica
		...
		mu.Unlock()
		go balance() // пытаемся восстановить нужное количество
	}
}
